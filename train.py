...
# make below part parametric
    ...
    # get loss for the predicted output
            ...
            loss = loss1+ 2 * l2_reg + 0.0005*(criterion(model(unknow_point[:400,:len(_X1[1])]), model2(unknow_point[:400,-(len(_X2[1])):]))) #co-regularsation term
...
def test_epochs(model,model2, test_set1,test_set2, criterion ,optimizer, restore_dir=None):
    ...
    for (Xb1, yb1),(Xb2 , yb2) in zip(test_set1,test_set2):
        ...
        total_error = (L1 + L2)/2
        test_batch_losses.append(total_error.item())
